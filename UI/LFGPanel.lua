...
-- ============================================================
-- LFG tab. Top: form to post your own "I'm here, here's my
-- class and rating, here's my note" card. Bottom: scrollable
-- feed of cards broadcast by other addon users in the hidden
-- "MeteoraArena" channel.
--
-- v0.2.2: simplified — no LFG/LFM distinction, no class wish
-- list. Just post who you are; whoever is interested whispers.
-- ============================================================
...
-- Skin:CreateButton attaches hover handlers that hard-reset backdrop —
-- this clobbers the "active" styling. For toggle buttons we override
-- OnEnter/OnLeave to respect a live predicate.
...
local function ensureRow(i, parent)
    ...
    row.when:SetPoint("LEFT", row, "LEFT", 6, 0); row.when:SetWidth(50); row.when:SetJustifyH("LEFT")

    row.who     = row:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
    row.who:SetPoint("LEFT", row.when, "RIGHT", 4, 0); row.who:SetWidth(180); row.who:SetJustifyH("LEFT")

    row.bracket = row:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
    row.bracket:SetPoint("LEFT", row.who, "RIGHT", 4, 0); row.bracket:SetWidth(70); row.bracket:SetJustifyH("LEFT")

    row.rating  = row:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
    row.rating:SetPoint("LEFT", row.bracket, "RIGHT", 4, 0); row.rating:SetWidth(70); row.rating:SetJustifyH("LEFT")

    row.comment = row:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
    row.comment:SetPoint("LEFT", row.rating, "RIGHT", 6, 0)
    row.comment:SetPoint("RIGHT", row, "RIGHT", -80, 0)
    row.comment:SetJustifyH("LEFT")
    row.comment:SetWordWrap(false)

    row.whisper = Skin:CreateButton(row, L["lfg_whisper"] or "Whisper", 72, 18)
...
local function buildForm(parent, yTop)
    ...
    bracketLabel:SetPoint("LEFT", form_bg, "TOPLEFT", 8, rowY - 11)
    ...
    for _, b_ in ipairs(BRACKETS) do
        local captured = b_
        local b = Skin:CreateButton(form_bg, captured, 70, 22)
    ...
    expLabel:SetPoint("LEFT", autoBtn, "RIGHT", 24, 0)
    ...
    cmtLabel:SetPoint("LEFT", form_bg, "TOPLEFT", 8, rowY - 11)
...
function LFGPanel:Build(parent)
    ...
    hCol(L["lfg_col_age"]     or "Age",     6,   50)
    hCol(L["lfg_col_who"]     or "Player",  60,  180)
    hCol(L["lfg_col_bracket"] or "Bracket", 244, 70)
    hCol(L["lfg_col_rating"]  or "Rating",  318, 70)
    hCol(L["lfg_col_comment"] or "Comment", 392, 200)
...
local function refreshForm()
    if not widgets.bracketBtns then return end
